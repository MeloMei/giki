...
- **Manual retry:** Transient LLM failures during the compilation pipeline require manual intervention using the `--retry-failed` flag.
...
- **`giki branch` / `giki pr` commands:** Native git branch and pull request management directly within the giki CLI.
...
- **`giki lint --fix`:** Automated linting that not only detects mechanical issues but proposes and applies fixes using the LLM.
